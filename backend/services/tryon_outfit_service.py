...
from backend.database.models import ClothingImage
from backend.services import leonardo_service, person_image_service


def resolve_clothing_image_urls(db: Session, clothing_image_ids: List[int]) -> List[str]:
    rows = (
...
def run_outfit_generation(db: Session, clothing_image_ids: List[int], extra_prompt: str | None) -> tuple[str, str]:
    ...
    garment_urls = resolve_clothing_image_urls(db, clothing_image_ids)
