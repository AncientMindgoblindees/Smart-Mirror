...
if [[ "${MIRROR_ENABLE_TUNNEL}" == "1" ]]; then
  if command -v cloudflared >/dev/null 2>&1 && [[ -f "${HOME}/.cloudflared/config.yml" ]]; then
    ...
    if [[ ! -f "${TUNNEL_PID_FILE}" ]]; then
      ...
      (
        cd "${ROOT_DIR}"
        exec cloudflared tunnel run "${MIRROR_TUNNEL_NAME}"
      ) >>"${TUNNEL_LOG_FILE}" 2>&1 &
    ...
    echo "Setup command:"
    echo "  bash ${ROOT_DIR}/scripts/setup-cloudflare-tunnel.sh --hostname mirror.yourdomain.com --service-url http://127.0.0.1:${PORT}"
  fi
fi
